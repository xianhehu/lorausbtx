...
static uint8_t ubx_msg_buf[256]={0};
...
{
    //free(buffer->data);
    memset(ubx_msg_buf, 0, sizeof(ubx_msg_buf));
}
...
UBXMsgBuffer getCFG_MSG_RATE(UBXU1_t msgClass, UBXU1_t msgId, UBXU1_t rate)
{
...
UBXMsgBuffer getCFG_PRT_UART(UBXU1_t portId, UBXU4_t baud)
{
    //TODO
    ...
    UBXMsg* msg = (UBXMsg*)buffer.data;
    initMsg(msg, payloadSize, UBXMsgClassCFG, UBXMsgIdCFG_PRT);
    msg->payload.CFG_PRT.portID=portId;
    msg->payload.CFG_PRT.option.UARTbaudRate=baud;
    msg->payload.CFG_PRT.mode.UART.charLen=3;
    msg->payload.CFG_PRT.mode.UART.parity=4;
    msg->payload.CFG_PRT.inProtoMask=0x3;
    msg->payload.CFG_PRT.outProtoMask=0x3;
    completeMsg(&buffer, payloadSize);
    return buffer;
}
...
UBXMsgBuffer getNAV_SAT_POLL()
{
    int payloadSize = 0;
    UBXMsgBuffer buffer = createBuffer(payloadSize);
    UBXMsg* msg = (UBXMsg*)buffer.data;
    initMsg(msg, payloadSize, UBXMsgClassNAV, UBXMsgIdNAV_SAT);
    completeMsg(&buffer, payloadSize);
...
UBXMsgAck getUBXMsgAck(uint8_t *data, uint16_t len)
{
    UBXMsgAck ack={0};
    uint16_t i=0;

    for (; i<len-7; i++) {
        uint16_t tmp=(*(data+i))<<8;
        tmp+=(*(data+i+1));
