...
enum { LINK_CHECK_CONT    =  32 ,    // continue with this after reported dead link
       LINK_CHECK_DEAD    =  64 ,    // after this UP frames and no response from NWK assume link is dead
       LINK_CHECK_INIT    = -32 ,    // UP frame count until we inc datarate
...
#elif defined(CFG_zte470) // EU868 spectrum ====================================================

enum { MAX_CHANNELS = 96 };      //!< Max supported channels
enum { MAX_BANDS    =  12 };
...
struct lmic_t {
    ...
    u2_t        channelMap;
#elif defined(CFG_zte470)
    u4_t        basefreq_tx;
    u4_t        basefreq_rx;
    u1_t        startband;                 // start band and band fixed flag
#if defined(CFG_zte470_support_bands)
    u1_t        join_band_seq[MAX_BANDS];
    band_t      bands[MAX_BANDS];
    u2_t        channelDrMap[MAX_CHANNELS];
    u2_t        channelMap[MAX_CHANNELS>>4];
#else
    band_t      bands[1];
    u2_t        channelDrMap[(u1_t)MAX_CHANNELS/MAX_BANDS];
    u2_t        channelMap[1];
#endif
