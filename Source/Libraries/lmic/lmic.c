...
#if defined(CFG_eu868) || defined(CFG_zte470)
...
static void aes_sessKeys (u2_t devnonce, xref2cu1_t artnonce, xref2u1_t nwkkey, xref2u1_t artkey) {
    ...
    nwkkey[0] = 0x01;
    os_copyMem(nwkkey+1, artnonce, (uint32_t)LEN_ARTNONCE+LEN_NETID);
...
static const u4_t iniChannelFreq_EU868[12] = {
...
static void initDefaultChannels (bit_t join) {
...
#endif
    {
        ...
        LMIC.bands[0].txcap    = 10;  // 0.1%
        LMIC.bands[0].txpow    = 20;
        LMIC.bands[0].lastchnl = os_getRndU1() % MAX_CHANNELS;
#endif
    }
#endif
}
...
bit_t LMIC_setupChannel (u1_t chidx, u4_t freq, u2_t drmap, s1_t band) {
...
#endif
    {
        ...
        if( freq >= (LMIC.startband+1)*1600000+ZTE470_FREQ_BASE_UL 
           || freq <= (LMIC.startband)*1600000+ZTE470_FREQ_BASE_UL - 200000 
           || freq % 200000 != 0)
            return 0;

        LMIC.channelDrMap[chidx] = drmap==0 ? DR_RANGE_MAP(DR_SF12,DR_SF7) : drmap;
        LMIC.channelMap[0] |= 1<<((freq - (LMIC.startband)*1600000+ZTE470_FREQ_BASE_UL)/200000);  // enabled right away
        return 1;
#endif
    }
#endif
...
#if defined(CFG_zte470_support_bands)
#else
        if (channel >= (LMIC.startband+1)*8
         || channel < LMIC.startband*8)
            return;
        channel -= LMIC.startband*8;
        LMIC.channelDrMap[channel] = 0;
...
#endif
    {
        if ( chpage != LMIC.startband>>1)
            return 0;
        if (chmap & (0xff << ((LMIC.startband&1)<<3)) == 0) 
            return 0;
        LMIC.channelMap[0] = chmap & (0xff << ((LMIC.startband&1)<<3));
        return 1;
    }
#endif
}
...
#endif
    {
        u4_t basefreq = LMIC.basefreq_tx > 0 ? LMIC.basefreq_tx : ZTE470_FREQ_BASE_UL;
        u4_t freq = LMIC.txChnl*200000+LMIC.startband*2000000+basefreq;
        // Update global/band specific duty cycle stats
        ostime_t airtime = calcAirTime(LMIC.rps, LMIC.dataLen);
        // Update channel/global duty cycle stats
        xref2band_t band = &LMIC.bands[0];
        LMIC.freq  = freq;
        LMIC.txpow = band->txpow;
        band->avail = txbeg + airtime * band->txcap;
        if( LMIC.globalDutyRate != 0 )
...
#else
        ostime_t mintime = now + /*10h*/36000*OSTICKS_PER_SEC; 
        mintime = LMIC.bands[0].avail;   
        // Find next channel in given band
        u1_t chnl = LMIC.bands[0].lastchnl;
        for( u1_t ci=0; ci<8; ci++ ) {
            if( (chnl = (chnl+1)) >= 8 )
                chnl %=  8;
            if( (LMIC.channelMap[0] & (1<<chnl)) != 0  &&  // channel enabled
                (LMIC.channelDrMap[chnl] & (1<<(LMIC.datarate&0xF))) != 0){ // in selected band
                LMIC.txChnl = LMIC.bands[0].lastchnl = chnl;
                return mintime;
            }
        }
...
static void initJoinLoop (void) {
...
#endif
    {
        ...
        LMIC.txChnl = 0;
        LMIC.adrTxPow = 20;
        LMIC.dn2Dr = LMIC.dn2SDr;
        setDrJoin(DRCHG_SET, DR_SF7);
        initDefaultChannels(1);
        ASSERT((LMIC.opmode & OP_NEXTCHNL)==0);
        LMIC.txend = LMIC.bands[0].avail + rndDelay(8);
#endif
    }
#endif
}
...
static ostime_t nextJoinState (void) {
...
#endif
    {
        ...
        // Duty cycle is based on txend.
        ostime_t time = os_getTime();
        if( time - LMIC.bands[0].avail < 0 )
            time = LMIC.bands[0].avail;
        LMIC.txend = time + DNW2_SAFETY_ZONE+rndDelay(255>>LMIC.datarate);
        // 1 - triggers EV_JOIN_FAILED event
        return failed;
#endif
    }
...
static void stateJustJoined (void) {
    ...
    //LMIC.dn2Dr       = DR_DNW2;
    if (LMIC.dn2Freq == 0) {
        LMIC.dn2Freq     = LMIC.startband*2000000+(LMIC.basefreq_rx==0?ZTE470_FREQ_BASE_DL:LMIC.basefreq_rx);
    }
...
#if defined(CFG_zte470)
    u4_t basefreq = LMIC.basefreq_rx > 0 ? LMIC.basefreq_rx : ZTE470_FREQ_BASE_DL;
    LMIC.freq = LMIC.txChnl*200000+LMIC.startband*2000000+basefreq;
#endif
    LMIC.dataLen = 0;
...
static bit_t processJoinAccept (void) {
    ...
    if( (dlen != LEN_JA && dlen != LEN_JAEXT)
        || (hdr & (HDR_FTYPE|HDR_MAJOR)) != ((u4_t)HDR_FTYPE_JACC|HDR_MAJOR_V1) ) {
    ...
    LMIC.rxdelay = LMIC.frame[OFF_JA_RXDLY]&0xF;
    LMIC.dn1DrOff = (LMIC.frame[OFF_JA_DLSET]>>4)&0x7;
    LMIC.dn2Dr = LMIC.frame[OFF_JA_DLSET]&0x7;
#if defined(CFG_eu868) || defined(CFG_zte470)
    ...
    // save key&devaddr to flash
    flash_param.devaddr=addr;
    memcpy(flash_param.appskey, LMIC.artKey, 16);
    memcpy(flash_param.nwkskey, LMIC.nwkKey, 16);
    flash_param.netstat=2;
    lmicapi_saveparam();
...
static void processRx1DnData (xref2osjob_t osjob) {
    if( LMIC.dataLen == 0 || !processDnData() )
        schedRx2(LMIC.rxdelay==0?DELAY_DNW2_osticks:sec2osticks(LMIC.rxdelay)+DELAY_DNW1_osticks,
...
static void updataDone (xref2osjob_t osjob) {
    txDone(LMIC.rxdelay==0?DELAY_DNW1_osticks:sec2osticks(LMIC.rxdelay), FUNC_ADDR(setupRx1DnData));
}
...
static void buildDataFrame (void) {
    ...
    LMIC.frame[OFF_DAT_HDR] = (u4_t)HDR_FTYPE_DAUP | HDR_MAJOR_V1;
    ...
    if( txdata ) {
        if( LMIC.pendTxConf ) {
            ...
            LMIC.frame[OFF_DAT_HDR] = (u4_t)HDR_FTYPE_DCUP | HDR_MAJOR_V1;
...
bit_t LMIC_startJoining (void) {
    //if( LMIC.devaddr == 0 ) {
        ...
        if (flash_param.valid==1) {
            LMIC.dn2Freq=flash_param.rx2freq;
            LMIC.dn2Dr=flash_param.rx2dr;
#ifdef CFG_zte470
            LMIC.startband=flash_param.band;
            LMIC.basefreq_tx=flash_param.txfreq;
            LMIC.basefreq_rx=flash_param.rxfreq;
#endif
        }
...
void LMIC_active(void)
{
    LMIC.devaddr=flash_param.devaddr;
    memcpy(LMIC.artKey, flash_param.appskey, 16);
    memcpy(LMIC.nwkKey, flash_param.nwkskey, 16);
    
    LMIC.rxdelay=flash_param.rxdelay;
    LMIC.datarate=flash_param.datarate;
    LMIC.dn1DrOff=flash_param.rxdroffset;
    LMIC.dn2Freq=flash_param.rx2freq;
    LMIC.dn2Dr=flash_param.rx2dr;
    ...
    LMIC.basefreq_rx=flash_param.rxfreq;
    LMIC.startband=flash_param.band;
    memcpy(LMIC.bands, flash_param.bands, sizeof(LMIC.bands));
#ifdef CFG_zte470_support_bands
...
void LMIC_reset (void) {
    ...
    LMIC.dn2SDr       =  ZTE470_DR_DNW2;
    LMIC.dn2Dr        =  ZTE470_DR_DNW2;   // we need this for 2nd DN window of join accept
    LMIC.dn2Freq      =  ZTE470_FREQ_DNW2; // ditto
    LMIC.ping.freq    =  ZTE470_FREQ_PING; // defaults for ping
