...
static unsigned int Q_Sh_ParseAnalysis(Q_SH_LEX_RESU_DEF *pQShLexResu)
{
	...
	switch(pQShLexResu->CallType)
	{
		case QSCT_CALL_FUN:
		{
			...
			if( strncmp(pRecord->desc, "void ", 5) )	//ydx 2016-07-04//
				printf("return %d\r\n",FunResu);
		}
		break;
...
char QShell_PSW_WaitInput = 0;	//密码输入状态标志
...
#if USE_Q_SHELL_PASSWORD
	static unsigned char IfPass=0,IfWaitInput=0;
#else
	static unsigned char IfPass=1,IfWaitInput=0;
#endif
	if(IfPass)
	{
		if(IfCtrl==1)
