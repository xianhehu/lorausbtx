...
#define GPS_BUFLEN		200
uint8 GpsBuf[GPS_BUFLEN];
uint8 GpsBufw = 0;
//======================================
//接收一个字节
//======================================
void GPS_RxdByte(uint8 bt)
{
	if(GpsBufw == 0)
	{
		if(bt == '$')
	...
	else
	{
		...
		if((bt == 0x0D) || (bt == 0x0A))
		{//接收到一帧数据包
			GpsBuf[GpsBufw ++] = 0;//自动添加结束符
			GPS_Analysis( &gpsx, GpsBuf);
			GpsBufw = 0;
		}
		else if(GpsBufw >= (GPS_BUFLEN-1))//溢出
			GpsBufw = 0;
	}
}
...
//       0XFF,代表不存在第cx个逗号							  
uint8 NMEA_Comma_Pos(uint8 *buf,uint8 cx)
{	 		    
	uint8 *p=buf;
	while(cx)
	{		 
...
void NMEA_GPGSV_Analysis(nmea_msg *gpsx,uint8 *buf)
{
	...
	uint8 posx;   	 
	p=buf;
	p1=(uint8*)strstr((const char *)p,"$GPGSV");
...
void NMEA_GPGGA_Analysis(nmea_msg *gpsx,uint8 *buf)
{
	uint8 *p1,dx;			 
	uint8 posx;    
	p1=(uint8*)strstr((const char *)buf,"$GPGGA");
...
void NMEA_GPGSA_Analysis(nmea_msg *gpsx,uint8 *buf)
{
	...
	uint8 posx; 
	uint8 i;   
	p1=(uint8*)strstr((const char *)buf,"$GPGSA");
...
void NMEA_GPRMC_Analysis(nmea_msg *gpsx,uint8 *buf)
{
	...
	uint32 temp;	   
	float rs;  
	p1=(uint8*)strstr((const char *)buf,"GPRMC");//"$GPRMC",经常有&和GPRMC分开的情况,故只判断GPRMC.
	...
	if(posx!=0XFF)
	{
		...
		gpsx->latitude=(uint32_t)(gpsx->latitude*NMEA_Pow(10,5)+(rs*NMEA_Pow(10,5-dx))/60);//转换为° 
	}
	...
	{												  
		...
		rs=temp%NMEA_Pow(10,dx+2);				//得到'		 
		gpsx->longitude=(uint32_t)(gpsx->longitude*NMEA_Pow(10,5)+(rs*NMEA_Pow(10,5-dx))/60);//转换为° 
	}
...
void NMEA_GPVTG_Analysis(nmea_msg *gpsx,uint8 *buf)
{
	uint8 *p1,dx;			 
	uint8 posx;    
	p1=(uint8*)strstr((const char *)buf,"$GPVTG");							 
...
//len:数据长度
//cka,ckb:两个校验结果.
void Ublox_CheckSum(uint8 *buf,uint16 len,uint8* cka,uint8*ckb)
{
	uint16 i;
	*cka=0;*ckb=0;
	for(i=0;i<len;i++)
...
//       2,没有找到同步字符
//       3,接收到NACK应答
uint8 Ublox_Cfg_Ack_Check(void)
{			 
	uint16 len=0,i;
	uint8 rval=0;
...
uint8 Ublox_Cfg_Cfg_Save(void)
{
	...
	Ublox_CheckSum((uint8*)(&cfg_cfg->id),sizeof(_ublox_cfg_cfg)-4,&cfg_cfg->cka,&cfg_cfg->ckb);
...
uint8 Ublox_Cfg_Msg(uint8 msgid,uint8 uart1set)
{
	...
	Ublox_CheckSum((uint8*)(&cfg_msg->id),sizeof(_ublox_cfg_msg)-4,&cfg_msg->cka,&cfg_msg->ckb);
	while(DMA1_Channel7->CNDTR!=0);	//等待通道7传输完成   
...
uint8 Ublox_Cfg_Prt(uint32 baudrate)
{
	...
	Ublox_CheckSum((uint8*)(&cfg_prt->id),sizeof(_ublox_cfg_prt)-4,&cfg_prt->cka,&cfg_prt->ckb);
	while(DMA1_Channel7->CNDTR!=0);	//等待通道7传输完成   
...
uint8 Ublox_Cfg_Tp(uint32 interval,uint32 length,signed char status)
{
	...
	Ublox_CheckSum((uint8*)(&cfg_tp->id),sizeof(_ublox_cfg_tp)-4,&cfg_tp->cka,&cfg_tp->ckb);
	while(DMA1_Channel7->CNDTR!=0);	//等待通道7传输完成   
...
uint8 Ublox_Cfg_Rate(uint16 measrate,uint8 reftime)
{
	...
	Ublox_CheckSum((uint8*)(&cfg_rate->id),sizeof(_ublox_cfg_rate)-4,&cfg_rate->cka,&cfg_rate->ckb);
	while(DMA1_Channel7->CNDTR!=0);	//等待通道7传输完成   
